...
import com.mod_int.carwash.databinding.FragmentWasherRegistrationPriceBinding

class WasherRegistrationPriceFragment : BaseFragment<FragmentWasherRegistrationPriceBinding>(
    R.layout.fragment_washer_registration_price) {
        ...
        user?.let {
            ...
            fireStore?.collection("headWasher")?.document(
                ...
                if (it.isSuccessful) {
                    ...
                    val toastCenter = Toast.makeText(washerActivity, "정보가 저장되지 않았습니다", Toast.LENGTH_SHORT)
                    toastCenter.setGravity(Gravity.CENTER, Gravity.CENTER_HORIZONTAL, 0)
                    toastCenter.show()
                }
            }
